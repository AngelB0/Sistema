...
def INDEX():
   ...
   #VENTANA PARA CONSULTAR
    def Consultar():
        ...
        class TreeviewFrame(ttk.Frame):
            def __init__(self, *args, **kwargs):
                ...
                self.treeview.place(x=0, y=0, width=1260, height=555)
    ...
    def confirmacion_pdf():
        ...
        mensaje_label = tk.Label(ventana_confirmacion, text="¿Deseas guardar un PDF?")
        mensaje_label.pack(padx=20, pady=20)

        # Botón "Sí" que realiza una acción y cierra la ventana
        boton_si = tk.Button(ventana_confirmacion, text="Sí", command=lambda: guardar_pdf(ventana_confirmacion))
        boton_si.pack(side=tk.LEFT, padx=10)

        # Botón "No" que cierra la ventana sin realizar ninguna acción
        boton_no = tk.Button(ventana_confirmacion, text="No", command=ventana_confirmacion.destroy)
        boton_no.pack(side=tk.RIGHT, padx=10)


    
    def guardar_datos_pdf():
    ...
    guardar_pdf_button = tk.Button(WIND, text="Guardar PDF", bg="#FF0000", fg="white", relief=tk.RAISED, bd=5, command=guardar_datos_pdf)
...
e1.grid(columnspan=2, row=2,padx=4, pady=4)

#CONTRASENA
contra = CTkLabel(frame, text="PASSWORD", font=("Candara", 20, "bold"), bg_color="#1D1212")
contra.grid(columnspan=2, row=3,padx=4, pady=4)

#CONTRASENA  ENTRADA 
e2 = CTkEntry(frame, font=font, show='*', placeholder_text='CONTRASENA',
              border_color="#4169e1", fg_color="#1D1212", width=220, height=40)
e2.grid(columnspan=2, row=4,padx=4, pady=4)
...
b1.grid(columnspan=2, row=6, padx=4, pady=4)
