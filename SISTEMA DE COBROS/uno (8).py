   ...
   #VENTANA PARA CONSULTAR
    def Consultar():
        ...
        class TreeviewFrame(ttk.Frame):
            def __init__(self, *args, **kwargs):
                ...
                self.combo_exportar = ttk.Combobox(self, values=["Descargar todas las Ventas", "Descargar solo por Usuario", "Descargar solo por Fecha"], foreground='green', width=25, font="Candara 11 bold", state="readonly")
                # Establecer un valor predeterminado (título por defecto)
                self.combo_exportar.set("Descargar Excel")
            ...
            def ejecutar_funcion_seleccionada(self, event):
                ...
                if opcion_seleccionada == "Descargar todas las Ventas":
                    self.exportar_a_excel()
                elif opcion_seleccionada == "Descargar solo por Usuario":
                    self.exportar_a_excel_con_filtro_usuario()
                elif opcion_seleccionada == "Descargar solo por Fecha":
    ...
    def enviar_datos():
        ...
        try:
            ...
            consulta = "INSERT INTO ventas (folio, plantel, matri, nombre, grupo, licenciatura, cant_recibe, forma_p, aprov, cuenta_rec, observaciones, subtotal, fecha, concep, usuario) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            # Imprimir la consulta y los valores antes de ejecutarla
            print("Consulta:", consulta)
            print("Valores:", (FOL, PLA, MATRI, NOMA, GRUP, LIC, CANTR, FP, APRO, CUR, OBSERVA, TOTAL, FE, CONCEPTOF, USU))
            cursor.execute(consulta, (FOL, PLA, MATRI, NOMA, GRUP, LIC, CANTR, FP, APRO, CUR, OBSERVA, TOTAL, FE, CONCEPTOF, USU))
    ...
    def times():
        ...
        clock.config(text=current_time, bg='#1D1212' ,fg="white",font=('calibri', 30, 'bold'))
    ...
    user1 = e1.get()
    enom= e0.get()
    nom = tk.Label(WIND, text="Bienvenido: " + enom, bg="#1D1212", bd=3, fg="white", font="Candara 14 bold")
    nom.place(x=850, y=50, width=400)
        
    #FOLIO
    tk.Label(WIND, text="FOLIO:", bg="#565151", fg="white", relief=tk.GROOVE, bd=3, font="Candara 16 bold").place(x=500, y=100, width=70, height=30)
    folio = tk.Text(WIND, height=1, width=10, wrap=tk.NONE, bd=0, padx=2, pady=2, font="Candara 14 bold")
    folio.place(x=600, y=100, width=100, height=30)
    folio.insert(tk.END, "")
    ...
    guardar_pdf_button = tk.Button(WIND, text="Guardar PDF", bg="#FF0000", fg="white", relief=tk.RAISED, bd=5, command=guardar_pdf)
    guardar_pdf_button.place(x=700, y=600, width=100, height=30)
...
boton_cerrar = tk.Button(ventana, text="Salir", command=cerrar_ventana)
